    ...
    <>
      ...
      <motion.button
        ...
        className={`fixed top-6 left-6 z-50 lg:hidden p-4 rounded-2xl transition-all duration-300 backdrop-blur-2xl border ${
          isDark 
            ? 'bg-slate-900/90 text-white border-teal-500/30 shadow-2xl shadow-teal-500/10' 
            : 'bg-white/90 text-slate-800 shadow-2xl border-teal-200/50'
        }`}
      ...
      <AnimatePresence>
        <motion.div
          ...
          className={`fixed left-0 top-0 h-full w-80 z-40 lg:translate-x-0 lg:opacity-100 flex flex-col ${
            isDark 
              ? 'bg-slate-900/95 backdrop-blur-3xl border-r border-teal-500/20 shadow-2xl shadow-teal-500/5' 
              : 'bg-white/95 backdrop-blur-3xl border-r border-teal-200/50 shadow-2xl'
          }`}
        >
