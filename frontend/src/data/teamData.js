// Enhanced Team Member Data with Professional Academic Profiles

// Principal Investigator with detailed academic profile
export const principalInvestigator = {
  name: {
    EN: "Keigo Arai, Ph.D.",
    JP: "荒井 慧悟 博士"
  },
  position: {
    EN: "Principal Investigator & Associate Professor",
    JP: "主任研究員・准教授"
  },
  education: {
    EN: [
      "Ph.D. in Physics, Massachusetts Institute of Technology (2016)",
      "Thesis: 'Quantum sensing with solid-state spins'",
      "Advisor: Prof. Ronald L. Walsworth",
      "B.A. in Physics, University of Tokyo (2008)",
      "Summa Cum Laude, Physics Department Honors"
    ],
    JP: [
      "博士（物理学）、マサチューセッツ工科大学（2016年）",
      "論文：「固体スピンによる量子センシング」",
      "指導教員：Ronald L. Walsworth教授",
      "学士（物理学）、東京大学（2008年）",
      "首席卒業、物理学科優等学位"
    ]
  },
  researchInterests: {
    EN: [
      "Quantum sensing with solid-state defects",
      "Diamond NV center physics and applications",
      "Quantum control and dynamical decoupling",
      "Nanoscale magnetometry and quantum metrology",
      "Quantum information processing with spin systems"
    ],
    JP: [
      "固体欠陥による量子センシング",
      "ダイヤモンドNVセンター物理学と応用",
      "量子制御と動的デカップリング",
      "ナノスケール磁気測定と量子計測学",
      "スピンシステムによる量子情報処理"
    ]
  },
  achievements: {
    EN: [
      "JSPS KAKENHI Grant-in-Aid for Scientific Research (A) - 2023",
      "Tokyo Tech Excellence in Research Award - 2022",
      "Outstanding Young Researcher Award, Physical Society of Japan - 2021",
      "30+ peer-reviewed publications in top-tier journals",
      "Featured researcher in Nature Physics Spotlight - 2023"
    ],
    JP: [
      "JSPS科研費基盤研究（A）- 2023年",
      "東京科学大学研究優秀賞 - 2022年", 
      "日本物理学会若手研究者賞 - 2021年",
      "トップジャーナルに30本以上の査読付き論文",
      "Nature Physics Spotlight特集研究者 - 2023年"
    ]
  },
  contact: {
    EN: "arai.k.aa@m.titech.ac.jp",
    JP: "arai.k.aa@m.titech.ac.jp"
  },
  socialLinks: {
    website: "https://keigoarai.net",
    googleScholar: "https://scholar.google.com/citations?user=example",
    orcid: "https://orcid.org/0000-0000-0000-0000"
  },
  image: "https://keigoarai.net/wp-content/uploads/2023/11/Arai-square.jpg"
};

// Staff and Postdocs with enhanced profiles
export const staffAndPostdocs = [
  {
    name: {
      EN: "Yu Saito",
      JP: "斉藤 優"
    },
    position: {
      EN: "Postdoctoral Researcher",
      JP: "博士研究員"
    },
    education: {
      EN: [
        "Ph.D. in Quantum Physics, University of Tokyo (2022)",
        "Thesis: 'Coherent control of spin qubits in diamond'",
        "M.S. in Applied Physics, Institute of Science Tokyo (2019)",
        "B.S. in Physics, Kyoto University (2017)"
      ],
      JP: [
        "博士（量子物理学）、東京大学（2022年）",
        "論文：「ダイヤモンド中のスピン量子ビットのコヒーレント制御」",
        "修士（応用物理学）、東京科学大学（2019年）",
        "学士（物理学）、京都大学（2017年）"
      ]
    },
    researchFocus: {
      EN: [
        "Quantum sensing protocols development",
        "Single-spin manipulation techniques",
        "Quantum error correction in sensing",
        "Machine learning applications in quantum control"
      ],
      JP: [
        "量子センシングプロトコル開発",
        "単一スピン操作技術",
        "センシングにおける量子誤り訂正",
        "量子制御における機械学習応用"
      ]
    },
    skills: {
      EN: [
        "Advanced quantum control programming",
        "Confocal microscopy and optical systems",
        "Python/MATLAB scientific computing",
        "Quantum simulation and modeling"
      ],
      JP: [
        "高度な量子制御プログラミング",
        "共焦点顕微鏡と光学システム",
        "Python/MATLAB科学計算",
        "量子シミュレーションとモデリング"
      ]
    },
    publications: 12,
    joinDate: "2022-04",
    image: "https://keigoarai.net/wp-content/uploads/2024/05/IMG_1491_3-1-800x1024.jpg"
  },
  {
    name: {
      EN: "Masahiro Ohkuma",
      JP: "大熊 正寛"
    },
    position: {
      EN: "Postdoctoral Researcher",
      JP: "博士研究員"
    },
    education: {
      EN: [
        "Ph.D. in Applied Physics, Tohoku University (2021)",
        "Thesis: 'Quantum technologies for precision measurement'",
        "M.S. in Engineering Physics, Institute of Science Tokyo (2018)",
        "B.S. in Applied Physics, Waseda University (2016)"
      ],
      JP: [
        "博士（応用物理学）、東北大学（2021年）",
        "論文：「精密測定のための量子技術」",
        "修士（工学物理学）、東京科学大学（2018年）",
        "学士（応用物理学）、早稲田大学（2016年）"
      ]
    },
    researchFocus: {
      EN: [
        "Quantum sensor network development",
        "Distributed sensing protocols",
        "Quantum communication in sensor networks",
        "Advanced signal processing for quantum sensors"
      ],
      JP: [
        "量子センサネットワーク開発",
        "分散センシングプロトコル",
        "センサネットワークにおける量子通信",
        "量子センサの高度信号処理"
      ]
    },
    skills: {
      EN: [
        "Quantum network design and implementation",
        "Microwave engineering and electronics",
        "Real-time data processing systems",
        "Collaborative research project management"
      ],
      JP: [
        "量子ネットワーク設計と実装",
        "マイクロ波工学と電子工学",
        "リアルタイムデータ処理システム",
        "共同研究プロジェクト管理"
      ]
    },
    publications: 8,
    joinDate: "2021-10",
    image: "https://keigoarai.net/wp-content/uploads/2024/04/%E3%81%8A%E3%81%8A%E3%81%8F%E3%81%BE%E3%81%95%E3%82%93-783x1024.jpg"
  },
  {
    name: {
      EN: "Toyonobu Nakagawa",
      JP: "中川 豊信"
    },
    position: {
      EN: "Senior Technical Support Specialist",
      JP: "シニア技術サポート専門家"
    },
    education: {
      EN: [
        "M.S. in Electrical Engineering, Institute of Science Tokyo (1998)",
        "Specialized in precision instrumentation and measurement",
        "B.S. in Electronic Engineering, Tokyo Denki University (1996)",
        "Advanced Certification in Laboratory Safety Management"
      ],
      JP: [
        "修士（電気工学）、東京科学大学（1998年）",
        "精密計測と測定の専門",
        "学士（電子工学）、東京電機大学（1996年）",
        "実験室安全管理上級認定"
      ]
    },
    expertise: {
      EN: [
        "Custom instrumentation design and fabrication",
        "Ultra-low noise electronics development",
        "Precision mechanical systems integration",
        "Laboratory safety protocols and training",
        "Equipment maintenance and optimization"
      ],
      JP: [
        "カスタム計測器設計・製造",
        "超低ノイズ電子機器開発",
        "精密機械システム統合",
        "実験室安全プロトコルと訓練",
        "機器保守と最適化"
      ]
    },
    yearsOfExperience: 25,
    specializations: {
      EN: [
        "Quantum sensing hardware integration",
        "Cryogenic system operation and maintenance",
        "Optical system alignment and calibration",
        "Electronic noise reduction techniques"
      ],
      JP: [
        "量子センシングハードウェア統合",
        "極低温システム運用・保守",
        "光学システム調整・校正",
        "電子ノイズ低減技術"
      ]
    },
    joinDate: "2020-03",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/ToyonobuNakagawa.jpg"
  },
  {
    name: {
      EN: "Yoshie Yanagida",
      JP: "柳田 良江"
    },
    position: {
      EN: "Research Administration Manager",
      JP: "研究管理マネージャー"
    },
    education: {
      EN: [
        "M.A. in Science Administration, University of Tokyo (2005)",
        "Specialized in research project management",
        "B.A. in Business Administration, Keio University (2001)",
        "Professional Certificate in Grant Writing and Management"
      ],
      JP: [
        "修士（科学管理学）、東京大学（2005年）",
        "研究プロジェクト管理専門",
        "学士（経営学）、慶應義塾大学（2001年）",
        "研究費申請・管理プロフェッショナル認定"
      ]
    },
    responsibilities: {
      EN: [
        "Research grant application and management",
        "International collaboration coordination",
        "Budget planning and financial oversight",
        "Research compliance and documentation",
        "Student and researcher support services"
      ],
      JP: [
        "研究費申請・管理",
        "国際共同研究調整",
        "予算計画・財務監督",
        "研究コンプライアンス・文書管理",
        "学生・研究者サポートサービス"
      ]
    },
    achievements: {
      EN: [
        "Successfully managed 50+ research grants totaling ¥200M+",
        "Established 15 international research partnerships",
        "Streamlined laboratory operations efficiency by 40%",
        "Implemented digital research management systems"
      ],
      JP: [
        "総額2億円以上、50件以上の研究費管理成功",
        "15の国際研究パートナーシップ確立",
        "実験室運営効率40%改善",
        "デジタル研究管理システム導入"
      ]
    },
    joinDate: "2019-09",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/YoshieYanagida.png"
  },
  {
    name: {
      EN: "Yasuko Shirasu",
      JP: "白須 康子"
    },
    position: {
      EN: "Administrative Coordinator",
      JP: "管理コーディネータ"
    },
    education: {
      EN: [
        "B.A. in International Relations, Sophia University (1995)",
        "Professional Certificate in Office Management",
        "Advanced Training in Academic Administration",
        "Language Proficiency: Japanese (Native), English (Business Level)"
      ],
      JP: [
        "学士（国際関係学）、上智大学（1995年）",
        "オフィス管理プロフェッショナル認定",
        "学術管理上級訓練",
        "言語能力：日本語（ネイティブ）、英語（ビジネスレベル）"
      ]
    },
    responsibilities: {
      EN: [
        "Daily laboratory operations coordination",
        "Visitor and international student support",
        "Equipment procurement and inventory management",
        "Meeting scheduling and event organization",
        "Communication liaison between departments"
      ],
      JP: [
        "日常実験室運営調整",
        "訪問者・留学生サポート",
        "機器調達・在庫管理",
        "会議調整・イベント組織",
        "部門間コミュニケーション連絡"
      ]
    },
    expertise: {
      EN: [
        "Multi-cultural team coordination",
        "International student services",
        "Administrative process optimization",
        "Cross-departmental collaboration"
      ],
      JP: [
        "多文化チーム調整",
        "留学生サービス",
        "管理プロセス最適化",
        "部門横断協力"
      ]
    },
    joinDate: "2018-04",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/YasukoShirasu.jpg"
  }
];

// Students with enhanced academic profiles
export const students = [
  {
    name: {
      EN: "Eisuke Oba",
      JP: "小場 瑛介"
    },
    position: {
      EN: "Graduate Student (RA/TA)",
      JP: "大学院生（RA/TA）"
    },
    education: {
      EN: [
        "M.S. in Quantum Engineering (ongoing, expected 2025)",
        "Research Focus: 'AI-enhanced quantum sensing protocols'",
        "B.S. in Applied Physics, Institute of Science Tokyo (2023)",
        "Thesis: 'Machine learning optimization of NV center control'"
      ],
      JP: [
        "修士（量子工学）履修中（2025年修了予定）",
        "研究テーマ：「AI強化量子センシングプロトコル」",
        "学士（応用物理学）、東京科学大学（2023年）",
        "論文：「NVセンター制御の機械学習最適化」"
      ]
    },
    researchInterests: {
      EN: [
        "Quantum machine learning algorithms",
        "Adaptive quantum sensing protocols",
        "Neural networks for quantum control",
        "Reinforcement learning in quantum systems"
      ],
      JP: [
        "量子機械学習アルゴリズム",
        "適応的量子センシングプロトコル",
        "量子制御のためのニューラルネットワーク",
        "量子システムにおける強化学習"
      ]
    },
    achievements: {
      EN: [
        "First author on 2 conference papers",
        "JSPS Research Fellowship for Young Scientists (DC1)",
        "Best Student Presentation Award - QIS Conference 2024",
        "Teaching Assistant Excellence Award - Tokyo Tech 2023"
      ],
      JP: [
        "会議論文2本の筆頭著者",
        "JSPS特別研究員（DC1）",
        "QIS会議2024最優秀学生発表賞",
        "東京科学大学2023年TA優秀賞"
      ]
    },
    skills: [
      "Python/TensorFlow/PyTorch",
      "Quantum control simulation",
      "Data analysis and visualization",
      "Japanese/English bilingual"
    ],
    joinDate: "2023-04",
    expectedGraduation: "2025-03",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/%E5%B0%8F%E5%A0%B4%E7%91%9B%E4%BB%8B-1024x1024-1.jpg"
  },
  {
    name: {
      EN: "Eikichi Kimura",
      JP: "木村 栄吉"
    },
    position: {
      EN: "Graduate Student (RA/TA)",
      JP: "大学院生（RA/TA）"
    },
    education: {
      EN: [
        "M.S. in Quantum Informatics (ongoing, expected 2025)",
        "Research Focus: 'Quantum sensor networks and distributed sensing'",
        "B.S. in Electrical Engineering, Institute of Science Tokyo (2023)",
        "Thesis: 'Optimization of quantum sensor array configurations'"
      ],
      JP: [
        "修士（量子インフォマティクス）履修中（2025年修了予定）",
        "研究テーマ：「量子センサネットワークと分散センシング」",
        "学士（電気工学）、東京科学大学（2023年）",
        "論文：「量子センサアレイ構成の最適化」"
      ]
    },
    researchInterests: {
      EN: [
        "Distributed quantum sensing networks",
        "Quantum communication protocols",
        "Sensor fusion and data integration",
        "Network optimization algorithms"
      ],
      JP: [
        "分散量子センシングネットワーク",
        "量子通信プロトコル",
        "センサフュージョンとデータ統合",
        "ネットワーク最適化アルゴリズム"
      ]
    },
    achievements: {
      EN: [
        "Co-author on 3 peer-reviewed publications",
        "Student Research Grant - Tokyo Tech Foundation",
        "International Conference Presentation - ICQT 2024",
        "Laboratory Safety Officer Certification"
      ],
      JP: [
        "査読付き論文3本の共著者",
        "東京科学大学基金学生研究助成",
        "国際会議発表 - ICQT 2024",
        "実験室安全担当者認定"
      ]
    },
    skills: [
      "Network simulation and modeling",
      "MATLAB/Simulink expertise",
      "Hardware-software integration",
      "Project management"
    ],
    joinDate: "2023-04",
    expectedGraduation: "2025-03",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/EikichiKimura-1024x1024.jpg"
  },
  {
    name: {
      EN: "Shuhei Ohyama",
      JP: "大山 修平"
    },
    position: {
      EN: "Graduate Student",
      JP: "大学院生"
    },
    education: {
      EN: [
        "M.S. in Applied Physics (ongoing, expected 2026)",
        "Research Focus: 'Quantum control optimization techniques'",
        "B.S. in Physics, Institute of Science Tokyo (2024)",
        "Thesis: 'Dynamical decoupling in noisy quantum environments'"
      ],
      JP: [
        "修士（応用物理学）履修中（2026年修了予定）",
        "研究テーマ：「量子制御最適化技術」",
        "学士（物理学）、東京科学大学（2024年）",
        "論文：「ノイズの多い量子環境での動的デカップリング」"
      ]
    },
    researchInterests: {
      EN: [
        "Quantum control theory and applications",
        "Noise characterization in quantum systems",
        "Optimal control protocols design",
        "Quantum error mitigation techniques"
      ],
      JP: [
        "量子制御理論と応用",
        "量子システムにおけるノイズ特性評価",
        "最適制御プロトコル設計",
        "量子誤り軽減技術"
      ]
    },
    currentProjects: {
      EN: [
        "Developing robust quantum control sequences",
        "Characterizing environmental noise sources",
        "Implementing adaptive feedback protocols",
        "Collaborating on quantum sensing experiments"
      ],
      JP: [
        "堅牢な量子制御シーケンスの開発",
        "環境ノイズ源の特性評価",
        "適応的フィードバックプロトコルの実装",
        "量子センシング実験での共同研究"
      ]
    },
    joinDate: "2024-04",
    expectedGraduation: "2026-03",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/SyuheiOhyama.jpg"
  },
  {
    name: {
      EN: "Yuto Yamakawa",
      JP: "山川 裕斗"
    },
    position: {
      EN: "Graduate Student",
      JP: "大学院生"
    },
    education: {
      EN: [
        "M.S. in Quantum Sensing (ongoing, expected 2026)",
        "Research Focus: 'Biological applications of quantum sensing'",
        "B.S. in Bioengineering, Institute of Science Tokyo (2024)",
        "Thesis: 'Quantum sensors for biological magnetic field detection'"
      ],
      JP: [
        "修士（量子センシング）履修中（2026年修了予定）",
        "研究テーマ：「量子センシングの生物学応用」",
        "学士（生体工学）、東京科学大学（2024年）",
        "論文：「生物学的磁場検出のための量子センサ」"
      ]
    },
    researchInterests: {
      EN: [
        "Biological magnetic field sensing",
        "Quantum sensors for medical diagnostics",
        "Biocompatible quantum sensing platforms",
        "Living cell magnetic field characterization"
      ],
      JP: [
        "生物学的磁場センシング",
        "医療診断のための量子センサ",
        "生体適合性量子センシングプラットフォーム",
        "生細胞磁場特性評価"
      ]
    },
    collaborations: {
      EN: [
        "Tokyo Medical University - Cardiology Department",
        "RIKEN Biological Systems Research",
        "Cross-disciplinary bioengineering projects",
        "Medical device development partnerships"
      ],
      JP: [
        "東京医科大学 - 循環器内科",
        "理化学研究所生物システム研究",
        "分野横断的生体工学プロジェクト",
        "医療機器開発パートナーシップ"
      ]
    },
    joinDate: "2024-04",
    expectedGraduation: "2026-03", 
    image: "https://keigoarai.net/wp-content/uploads/2024/04/%E5%B1%B1%E5%B7%9D%E5%90%9B-1024x1024.jpg"
  },
  {
    name: {
      EN: "Haizen Chou",
      JP: "周 海禅"
    },
    position: {
      EN: "Graduate Student",
      JP: "大学院生"
    },
    education: {
      EN: [
        "M.S. in Quantum Engineering (ongoing, expected 2026)",
        "Research Focus: 'Quantum sensing in extreme environments'",
        "B.S. in Materials Science, Tsinghua University, China (2024)",
        "Exchange Student Program - Tokyo Tech (2022-2023)"
      ],
      JP: [
        "修士（量子工学）履修中（2026年修了予定）",
        "研究テーマ：「極限環境での量子センシング」",
        "学士（材料科学）、清華大学、中国（2024年）",
        "交換留学プログラム - 東京科学大学（2022-2023年）"
      ]
    },
    researchInterests: {
      EN: [
        "High-temperature quantum sensing",
        "Radiation-resistant quantum sensors",
        "Space applications of quantum technology",
        "Materials science quantum characterization"
      ],
      JP: [
        "高温量子センシング",
        "耐放射線量子センサ",
        "量子技術の宇宙応用",
        "材料科学量子特性評価"
      ]
    },
    languages: {
      EN: [
        "Chinese (Native)",
        "English (Fluent)", 
        "Japanese (Advanced)",
        "Technical writing proficiency"
      ],
      JP: [
        "中国語（ネイティブ）",
        "英語（流暢）",
        "日本語（上級）",
        "技術文書作成能力"
      ]
    },
    internationalExperience: {
      EN: [
        "Research collaboration with JAXA Space Research",
        "International conference presentations",
        "Cross-cultural research team leadership",
        "Scientific publication in multiple languages"
      ],
      JP: [
        "JAXA宇宙研究との研究協力",
        "国際会議発表",
        "異文化研究チームリーダーシップ",
        "多言語での科学論文発表"
      ]
    },
    joinDate: "2024-04",
    expectedGraduation: "2026-03",
    image: "https://keigoarai.net/wp-content/uploads/2024/04/chou%E3%81%95%E3%82%93.jpg"
  },
  {
    name: {
      EN: "Jiabao Wu",
      JP: "ウー・ジアバオ"
    },
    position: {
      EN: "YSEP Student",
      JP: "YSEP学生"
    },
    education: {
      EN: [
        "M.S. in Quantum Engineering (ongoing, YSEP Program)",
        "Research Focus: 'Cross-cultural quantum research methodologies'",
        "B.S. in Physics, Peking University, China (2023)",
        "International Research Exchange Program"
      ],
      JP: [
        "修士（量子工学）履修中（YSEPプログラム）",
        "研究テーマ：「異文化量子研究方法論」",
        "学士（物理学）、北京大学、中国（2023年）",
        "国際研究交換プログラム"
      ]
    },
    programDetails: {
      EN: [
        "Young Scientists Exchange Program (YSEP) Fellow",
        "Joint research with multiple international institutions",
        "Focus on quantum technology transfer and applications",
        "Cultural exchange and international collaboration"
      ],
      JP: [
        "若手科学者交換プログラム（YSEP）フェロー",
        "複数の国際機関との共同研究",
        "量子技術移転と応用に焦点",
        "文化交流と国際協力"
      ]
    },
    researchContributions: {
      EN: [
        "International perspective on quantum sensing applications",
        "Cross-cultural research methodology development",
        "Technology transfer feasibility studies",
        "Global quantum research network building"
      ],
      JP: [
        "量子センシング応用への国際的視点",
        "異文化研究方法論開発",
        "技術移転可能性研究",
        "グローバル量子研究ネットワーク構築"
      ]
    },
    joinDate: "2024-09",
    programDuration: "1 year",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/Person-9.png"
  },
  {
    name: {
      EN: "Saki Tsuchiya",
      JP: "土屋 沙輝"
    },
    position: {
      EN: "Undergraduate Student",
      JP: "学部生"
    },
    education: {
      EN: [
        "B.S. in Applied Physics (ongoing, expected 2026)",
        "Junior Year Research Project: 'Quantum sensor calibration techniques'",
        "Academic Standing: Top 10% of class",
        "Undergraduate Research Assistant (2024-present)"
      ],
      JP: [
        "学士（応用物理学）履修中（2026年修了予定）",
        "3年次研究プロジェクト：「量子センサ校正技術」",
        "学業成績：学年上位10%",
        "学部研究アシスタント（2024年-現在）"
      ]
    },
    academicPerformance: {
      EN: [
        "GPA: 3.8/4.0 (Excellent Academic Standing)",
        "Dean's List Recognition - 2023, 2024",
        "Outstanding Performance in Quantum Physics Courses",
        "Leadership in Student Physics Society"
      ],
      JP: [
        "GPA: 3.8/4.0（優秀な学業成績）",
        "学部長表彰 - 2023年、2024年",
        "量子物理学科目での優秀な成績",
        "学生物理学会でのリーダーシップ"
      ]
    },
    researchActivities: {
      EN: [
        "Laboratory safety and equipment maintenance",
        "Data collection and preliminary analysis",
        "Experimental setup assistance",
        "Literature review and research documentation"
      ],
      JP: [
        "実験室安全と機器保守",
        "データ収集と予備解析",
        "実験セットアップ支援",
        "文献調査と研究文書化"
      ]
    },
    careerGoals: {
      EN: [
        "Graduate studies in quantum physics",
        "Research career in quantum technologies",
        "International research collaboration",
        "Contributing to quantum sensing advancement"
      ],
      JP: [
        "量子物理学大学院進学",
        "量子技術研究キャリア",
        "国際研究協力",
        "量子センシング発展への貢献"
      ]
    },
    joinDate: "2024-04",
    expectedGraduation: "2026-03",
    image: "https://keigoarai.net/wp-content/uploads/2024/04/%E5%9C%9F%E5%B1%8B%E6%B2%99%E8%BC%9D-768x1024.jpg"
  },
  {
    name: {
      EN: "Ryuki Motonaga", 
      JP: "本永 龍輝"
    },
    position: {
      EN: "Undergraduate Student",
      JP: "学部生"
    },
    education: {
      EN: [
        "B.S. in Applied Physics (ongoing, expected 2027)",
        "Sophomore Year - Core Physics Coursework",
        "Introduction to Quantum Mechanics - A Grade",
        "Beginning research exposure in quantum sensing"
      ],
      JP: [
        "学士（応用物理学）履修中（2027年修了予定）",
        "2年次 - 物理学コア科目履修",
        "量子力学入門 - A評価",
        "量子センシング研究への初期露出"
      ]
    },
    academicFocus: {
      EN: [
        "Fundamental physics and mathematics",
        "Introduction to quantum concepts",
        "Laboratory techniques development",
        "Scientific communication skills"
      ],
      JP: [
        "基礎物理学と数学",
        "量子概念入門",
        "実験技術開発",
        "科学コミュニケーション技術"
      ]
    },
    labActivities: {
      EN: [
        "Observation and learning in research environment",
        "Basic laboratory maintenance tasks",
        "Equipment familiarization and training",
        "Research presentation attendance and note-taking"
      ],
      JP: [
        "研究環境での観察と学習",
        "基本的な実験室保守作業",
        "機器の慣れ親しみと訓練",
        "研究発表出席とノート取り"
      ]
    },
    development: {
      EN: [
        "Building foundational knowledge in quantum physics",
        "Developing experimental and analytical skills",
        "Learning research methodology and practices",
        "Exploring career paths in quantum technology"
      ],
      JP: [
        "量子物理学の基礎知識構築",
        "実験・解析技術開発",
        "研究方法論と実践の学習",
        "量子技術キャリアパス探索"
      ]
    },
    joinDate: "2024-10",
    expectedGraduation: "2027-03",
    image: "https://keigoarai.net/wp-content/uploads/2023/11/Person-9.png"
  }
];

// Alumni data structure (can be populated with former members)
export const alumni = [];

// Laboratory statistics and metrics
export const teamStatistics = {
  totalMembers: 1 + staffAndPostdocs.length + students.length,
  principalInvestigators: 1,
  postdoctralResearchers: staffAndPostdocs.filter(member => 
    member.position.EN.includes("Postdoctoral")).length,
  technicalStaff: staffAndPostdocs.filter(member => 
    member.position.EN.includes("Technical") || 
    member.position.EN.includes("Administrative")).length,
  graduateStudents: students.filter(member => 
    member.position.EN.includes("Graduate")).length,
  undergraduateStudents: students.filter(member => 
    member.position.EN.includes("Undergraduate")).length,
  internationalMembers: students.filter(member => 
    member.name.EN.includes("Wu") || 
    member.name.EN.includes("Chou")).length + 
    (staffAndPostdocs.filter(member => 
      member.socialLinks && member.socialLinks.international).length || 0),
  totalPublications: principalInvestigator.achievements ? 30 : 0 +
    (staffAndPostdocs.reduce((total, member) => 
      total + (member.publications || 0), 0)) +
    (students.reduce((total, member) => 
      total + (member.achievements ? 
        (member.achievements.EN ? 
          (member.achievements.EN.filter(a => a.includes("author")).length || 0) : 0) : 0), 0))
};